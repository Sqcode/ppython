...
def valid(html, url):
    ...
    while(True):
        ...
        if len(titles) > 0:
            ...
            # return html
        if times > 3:
            raise MyException('无法通过滑块验证，error')
...
def do_scrawl(url, movie_name=f'movie{str(round(time.time() * 1000))}'):
    ...
    #     save_to_txt(url, root_path + f'files/{movie_name}.txt')
...
# 多线程爬取
def save_batch(start, end, filepath):
    ...
    for i in range(start, end):
        # 反爬
        time.sleep(1 + float(random.randint(1,100)) / 20)
        url = f'http://m.maoyan.com/mmdb/comments/movie/1263235.json?_v_=yes&offset={i*size+1}'
...
if __name__ =='__main__':
    # thread_test("movie_name")
    # t = ThreadCrawl('线程', add, 3)
    # t.start()
    # print(1 + float(random.randint(1, 100)) / 20)
